...
import matplotlib.pyplot as plt
from load_annotations import load_mat
import argparse
...
if __name__=="__main__":
    parser = argparse.ArgumentParser(
        description='Look at the annotations of an image you choose')
    
    parser.add_argument('--image_to_display_index', action='store', default=0, type = int,
                        help='Index of the image you want to see the labels into training set')

    parser.add_argument('--dataset_dir', action='store', default='dir', 
                        help='root of the dataset')
    annotations=load_mat("/content/emotic/Annotations/Annotations/Annotations.mat")
    args=parser.parse_args()
    training_set=annotations["train"]
    test_image=training_set[args.image_to_display_index]
    Image= io.imread(args.dataset_dir+str(test_image["folder"])+"/"+str(test_image["filename"]))
    if type(test_image["person"]) is not dict:
        displayAnnotation_multiple(I=Image,persons=test_image["person"])
        print("Multiple People")    
    else:
        print("Single Person")
        displayAnnotation(I=Image,body_bbox=test_image["person"]["body_bbox"], anns_con=test_image["person"]["annotations_continuous"], anns_cat=test_image["person"]["annotations_categories"]["categories"], personID=10, Npeople=len(test_image["person"]), gender=test_image["person"]["gender"], age=test_image["person"]["age"])
